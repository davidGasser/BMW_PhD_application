...
## Setup 
Since my PC is a literal potato, I had to rely on notebooks and Google Colab to run the tasks. Therefore the setup is straight forward as you simply have to run through the notebook. Everything should work as is. 
...
To clean the data, as there were quite a few noisy elements, I first split the paragraphs to keep the structure, and then split the sentences to their elements to get rid of all noise. I also ignored tables where possible as their format is not ideal for training data. I decided to save the data as '.parquet' as this offered great compression for the long strings. 

### 2. Data Preparation
I chose to use only a small amount of press releases as the training was extremely slow as is and I was struggeling to finish even a full epoch with the computing time that I had avaiable. Therefore, taking a very small dataset made sense, but the general setup should scale for larger ones. 
...
I decided to use the HuggingFace libraries in general as they, in my opinion, provided the ideal abstraction level needed for this task. There was no reason to implement everything in pytorch. 
...
The training curves show that the models are generally learning and validate that the training pipeline is working. The curves are reletively stable, however there is some randomness in the initiation at time that leads to slightly different results and worse learning than seen here. This behavior and the curves in general could definitely be optimized further, but as I understood from the assignment perfect curves are not the goal. 

The reduced model is performing significantly worse on an absolute level. But looking at the margin of improvement, it is clear that it is learning quicker than its counterpart. What is intersting though and what I did not expect is that the shapes of the training curves are so similar. They have an offset and a different angle, but they have valleys and peaks at the same points. As the datasets were not randomized between the two, I suppose that certain samples are the reasons for this behavior. Even taking that into account, I did not expect the reaction to be almost the exact same.

___
#### Eval Curve
![alt text](imgs/eval_loss.png)
The eval curves are also stable and validate the overall pipeline. For their absolute value, they are slightly higher than their training counterpart which tracks and altough there are only a few steps, there is a clear trend of the model generalizing quite well. The curves are slightly smoother as the eval steps are less often than the logging steps. 
___
#### Test Perplexity

To compare the untrained model, the trained full model, and the trained reduced model I used the perplexity score as it is the golden standard for next token prediction. (The untrained reduced model does not really make much sense as the neuron connection are not adjusted before training).
